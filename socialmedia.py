...
def main():
    ...
    #users
"""    print("Adding a user into my mongo collection users")
    user_info = { "username": "arifcalik", "password": "1facebook2", "email" : "arifcalik123@otmail.com" }
    result = mycollection_name_users.insert_one(user_info)
    pp.pprint(result.inserted_id)
    pp.pprint("")
    user_info = { "username": "aysefatma", "password": "12345678", "email" : "aysefatma@hotmail.com" }
    result = mycollection_name_users.insert_one(user_info)    
    pp.pprint(result.inserted_id)
    pp.pprint("")
 
    #posts
    print("Adding a post into my mongo collection posts")
 
    #datetime_now = datetime.utcnow()
    #new_doc = db.content.insert_one({"updated": datetime_now})
 
    post_info = { "title": "title","content": "content", "likes": 0, "view" : 0 , "date": date}
    result = mycollection_name_posts.insert_one(post_info)
    pp.pprint(result.inserted_id)
    pp.pprint("")
    post_info = { "content": "content", "likes": 5, "view" : 17 }
    result = mycollection_name_posts.insert_one(post_info)    
    pp.pprint(result.inserted_id)
    pp.pprint("")"""
